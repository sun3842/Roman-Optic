<script type="text/javascript" rel="script">
    ...
    $(function () {
        $("#slider-range").slider({
            ...
            min: 12,
            max: 100,
            values: [<?php echo ($offer[0]['condition_starting_age']=='')?25:$offer[0]['condition_starting_age']?>, <?php echo ($offer[0]['condition_ending_range']=='')?60:$offer[0]['condition_ending_range']?>],
    ...
    function offer_product() {console.log(availableTags);
    ...
    $('#offer_products_search').keyup(function () {
        ...
        if(product_name!=''){
            $.ajax({
                url: '<?php echo site_url('add_offer');?>',
                type: 'POST',
    ...
    $('.btn-delete-offer-img').click(function () {
        var offer_img_id=$(this).attr('content');
        $(this).parent().remove();

        $.ajax({
            url: '<?php echo site_url('edit_offer/'.$offer[0]["offer_id"])?>',
            type: 'POST',
            data: {offer_img:offer_img_id},
            success: function (result) {
                    if(result==0){
    ...
    $('.btn-delete-offer-product').click(function () {
        var offer_product_id=$(this).attr('content');
        $(this).parent().remove();

        $.ajax({
            url: '<?php echo site_url('edit_offer/'.$offer[0]["offer_id"])?>',
            type: 'POST',
            data: {offer_product:offer_product_id},
            success: function (result) {
                if(result==0){
    ...
    offer_city_id= <?php echo ($offer[0]['condition_cities_id']!='')?$offer[0]['condition_cities_id']:-1;?>;
